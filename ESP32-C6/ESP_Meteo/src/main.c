...
static esp_err_t thingspeak_send_readings(float temperature_c, float humidity_pct, float pressure_hpa)
{
    // Convert hPa -> mmHg
    ...
    int written = snprintf(
        url,
        sizeof(url),
        "https://api.thingspeak.com/update?api_key=%s&field1=%.2f&field2=%.2f&field3=%.2f",
        THINKSPEAK_API_KEY,
        temperature_c,
        humidity_pct,
        pressure_mmhg);
...
void app_main() {
    ...
    while (count < 10) {
        ...
        if (count == 9) {
            ...
            } else if (!(ok_temp && ok_hum && ok_press)) {
                ESP_LOGW(TAG, "Skipping ThingSpeak update: missing sensor readings (t=%d h=%d p=%d)",
                         ok_temp, ok_hum, ok_press);
            } else {
                (void)thingspeak_send_readings(temperature, humidity, pressure);
            }
        }
        count++;
    }
